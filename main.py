...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(args.start_epochs, args.epochs)):
        ...
        global_model.load_global_model(
            FedAvg(local_models, local_nums), args.device, args.gem
        )
        ...
        if (epoch + 1) % args.test_interval == 0:
            ...
            printf(
                "Testing accuracy: Top1: {:.3f}, Top5: {:.3f}".format(
                    acc_test, acc_test_top5
                ),
                log_path,
            )
            if acc_best is None or acc_test >= acc_best:
                ...
                if args.save:
                    torch.save(
                        global_model.state_dict(), args.save_dir + "model_best.pth"
                    )
        ...
        if args.watermark:
            ...
            while (
                watermark_acc <= 98
                and watermark_embed_iters <= args.watermark_max_iters
            ):
                global_model.train()
            ...
            if args.fingerprint:
                for client_idx in range(len(clients)):
                    ...
                    printf(
                        "(Client_idx:{}, Result_idx:{}, FSS:{})".format(
                            client_idx, extract_idx, fss
                        ),
                        log_path,
                    )
        ...
        if (epoch + 1) % args.test_interval == 0:
            if args.watermark:
                ...
                for client_idx in range(args.num_clients):
                    acc, acc_top5 = test_img(
                        clients[client_idx].model, test_dataset, args
                    )
                    watermark_acc, _ = test_img(
                        clients[client_idx].model, trigger_set, args
                    )
                    ...
                    if args.fingerprint:
                        embed_layers = get_embed_layers(
                            clients[client_idx].model, args.embed_layer_names
                        )
                        fss, extract_idx = extracting_fingerprints(
                            embed_layers, local_fingerprints, extracting_matrices
                        )
                    ...
                    if acc >= client_acc_best[client_idx]:
                        ...
                        if args.save:
                            torch.save(
                                clients[client_idx].get_model().state_dict(),
                ...
                printf(
                    "Clients Average Testing accuracy: {:.2f}".format(avg_acc), log_path
                )
                printf(
                    "Clients Quantile Testing accuracy, Top1: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}".format(
                        min_acc, low_acc, median_acc, high_acc, max_acc
                    ),
                    log_path,
                )
                printf(
                    "Clients Quantile Testing accuracy, Top5: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}".format(
                        min_acc_top5,
                        low_acc_top5,
                        median_acc_top5,
                        high_acc_top5,
                ...
                printf(
                    "Watermark Average Testing accuracy:{:.2f}".format(
                        avg_watermark_acc
                    ),
                    log_path,
                )
    ...
    if args.watermark:
        ...
        printf(
            "Clients Quantile Testing accuracy: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}".format(
                min_acc, low_acc, median_acc, high_acc, max_acc
            ),
            log_path,
        )
